...
class LayoutBuilder {
   ...
   */
  layoutDocument(docStructure, pdfDocument, styleDictionary, defaultStyle, background, header, footer, watermark, pageBreakBeforeFct, sticky) {
  ...
  tryLayoutDocument(docStructure, pdfDocument, styleDictionary, defaultStyle, background, header, footer, watermark, sticky) {
    ...
    if (isNecessaryAddFirstPage(docStructure)) {
      this.writer.addPage(this.pageSize, null, this._docPageMargins, this._docCustomProperties);
    }
  ...
  resolveSticky(sticky, pageSize, pageMargins) {
    ...
    let resolveHeight = region => {
      ...
      return (0, _variableType.isNumber)(node.height) ? node.height : this.measureStickyHeight(node, innerWidth);
    };
  ...
  measureStickyHeight(node, width) {
    ...
    let clone = (0, _tools.cloneContent)(node);
    ...
    ['left', 'right', 'above', 'below'].forEach(side => {
      if (sticky[side] !== undefined) {
        if (sticky[side] === null || typeof sticky[side] === 'function') {
          ...
          cp['sticky.' + side] = (0, _tools.convertToDynamicContent)(sticky[side]);
        }
      }
    });
    return cp;
  ...
  addStickyRegions() {
    let dimsForCurrentPage = () => this.writer.context().getCurrentPage().customProperties.stickyDims || {
      leftW: 0,
      rightW: 0,
      aboveH: 0,
      belowH: 0
  ...
  processSection(sectionNode) {
    ...
    if (!page || page && page.items.length) {
      ...
      this.writer.addPage(sectionPageSize, sectionNode.pageOrientation, this.insetMarginsForSticky(sectionPageMargins, sectionStickyDims), customProperties);
      ...
      let sectionStickyDims = this.resolveSticky(sectionNode.sticky, sectionPageSize, sectionPageMargins);
      let customProperties = this.stickyCustomProperties(sectionNode.sticky, sectionStickyDims, {
